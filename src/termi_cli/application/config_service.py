"""Application-level service cho các tác vụ cấu hình.

Tách logic chọn model, quản lý persona, custom instructions, profile và diagnostics
ra khỏi handler để handler chỉ còn vai trò I/O/CLI.
"""
...
from termi_cli.infrastructure.config_repository import ConfigRepository


class ConfigService:
    """Service chính cho các thao tác cấu hình của Termi CLI."""

    def __init__(self, repository: ConfigRepository | None = None) -> None:
        """Khởi tạo service với một ConfigRepository để thao tác IO config.

        Cho phép inject repository khác khi test, mặc định dùng ConfigRepository
        gắn với config.json hiện tại.
        """
    ...
    def model_selection_wizard(self, console: Console, config: dict) -> None:
        """UI chọn model nâng cao: default_model, code_model, commit_model.

        Bước đầu tiên: chọn provider (Gemini / DeepSeek / Groq / OpenRouter / Ollama),
        ...
        table.add_column(
            "Mô tả" if language == "vi" else "Description", style="magenta"
        )
        ...
        def _print_provider_hint(model_name: str) -> None:
            ...
            if model_name.startswith("deepseek-"):
                ...
                console.print(
                    i18n.tr(language, "config_model_provider_hint_openrouter")
                )
        ...
        def _select_index(
            ...
            while True:
                try:
                    choice_str = console.input(
                        i18n.tr(language, prompt_key), markup=False
                    ).strip()
        ...
        if provider_key == "gemini":
            ...
            stable_models = sorted(
                [m for m in models if "preview" not in m and "exp" not in m]
            )
            preview_models = sorted(
                [m for m in models if "preview" in m or "exp" in m]
            )
            ...
            console.print(
                i18n.tr(language, "config_default_model_set", model=default_model)
            )
            ...
            console.print(
                i18n.tr(
                    language,
                    "config_commit_model_same_as_code_or_default",
                )
            )
        ...
        # --- Nhánh Ollama: nhập tag model thủ công hoặc chọn từ `ollama list` ---
        if provider_key == "ollama":
            ...
            def _ask_ollama_model(prompt_key: str, prefix: str) -> str | None:
                while True:
                    try:
                        ...
                        if not choice_str:
                            console.print(
                                i18n.tr(language, "config_selection_cancelled")
                            )
                            return None
            ...
            variant_raw = console.input(
                i18n.tr(language, "config_ollama_variant_prompt"), markup=False
            ).strip()
            variant = 2 if variant_raw == "2" else 1

            if variant == 2:
                console.print(i18n.tr(language, "config_ollama_cloud_api_hint"))
                default_model = _ask_ollama_model(
                    "config_ollama_cloud_model_prompt", "ollama-cloud/"
                )
            else:
                local_models = _get_ollama_local_models()
                if local_models:
                    table = Table(
                        title=i18n.tr(language, "config_model_selection_title")
                    )
                    table.add_column("#", style="cyan")
                    ...
                    for idx, item in enumerate(local_models, start=1):
                        size_display = (
                            f"{item['size_mb']:.1f}" if item["size_mb"] else "-"
                        )
                    ...
                    while True:
                        try:
                            prompt = i18n.tr(
                                language, "config_ollama_local_select_prompt"
                            )
                            choice_str = console.input(prompt, markup=False).strip()
                            if not choice_str:
                                console.print(
                                    i18n.tr(
                                        language, "config_selection_cancelled"
                                    )
                                )
                                return None
                    ...
                    default_model = _ask_ollama_model(
                        "config_ollama_default_model_prompt", "ollama/"
                    )
        ...
        if provider_key == "deepseek":
            ...
            def _openrouter_usage_hint(model_name: str) -> str:
                if model_name == "openai/gpt-4o-mini":
                    return i18n.tr(
                        language, "config_model_hint_openrouter_gpt4o_mini"
                    )
                if model_name == "meta-llama/llama-3.1-8b-instruct":
                    return i18n.tr(
                        language, "config_model_hint_openrouter_llama_8b"
                    )
                if model_name == "meta-llama/llama-3.1-70b-instruct":
                    return i18n.tr(
                        language, "config_model_hint_openrouter_llama_70b"
                    )
                if model_name == "mistralai/mixtral-8x7b-instruct":
                    return i18n.tr(
                        language, "config_model_hint_openrouter_mixtral"
                    )
                return ""
            ...
            for i, model_name in enumerate(sorted_models):
                table.add_row(
                    ...
                    _openrouter_usage_hint(model_name),
                )
            console.print(table)
            console.print(i18n.tr(language, "config_openrouter_intro_examples"))

            def _ask_openrouter_model(
                prompt_key: str, allow_blank: bool, baseline: str | None = None
            ) -> str | None:
                while True:
                    try:
                        choice_str = console.input(
                        ...
                        if not api.is_openrouter_model(choice_str):
                            console.print(
                                i18n.tr(
                                    language,
                                    "config_openrouter_invalid_model",
                                )
                            )
                            continue
                        return choice_str
            ...
            default_model = _ask_openrouter_model(
                "config_openrouter_default_model_prompt",
                allow_blank=False,
            )
            if default_model is None:
                console.print(i18n.tr(language, "config_selection_cancelled"))
        ...
        def _static_model_usage_hint(model_name: str) -> str:
            if provider_key == "deepseek":
                if model_name == "deepseek-chat":
                    return i18n.tr(
                        language, "config_model_hint_deepseek_chat"
                    )
                if model_name == "deepseek-reasoner":
                    return i18n.tr(
                        language, "config_model_hint_deepseek_reasoner"
                    )
            elif provider_key == "groq":
                ...
                if model_name == "groq-llama-3.3-70b-versatile":
                    return i18n.tr(
                        language,
                        "config_model_hint_groq_llama_3_3_70b",
                    )
                if model_name == "groq-llama3-8b-8192":
                    return i18n.tr(
                        language, "config_model_hint_groq_llama3_8b"
                    )
            return ""
        ...
        for i, model_name in enumerate(sorted_models):
            table.add_row(
                str(i + 1),
                _display_name(model_name),
                _static_model_usage_hint(model_name),
            )
        console.print(table)
    ...
    def add_instruction(self, console: Console, config: dict, instruction: str) -> None:
        language = config.get("language", "vi")
    ...
    def list_instructions(self, console: Console, config: dict) -> None:
    ...
    def remove_instruction(
        self, console: Console, config: dict, index: int
    ) -> None:
        instructions = config.get("saved_instructions", [])
        language = config.get("language", "vi")
        if not 1 <= index <= len(instructions):
            console.print(
                i18n.tr(
                    language,
                    "config_invalid_instruction_index",
    ...
    def add_persona(
        self, console: Console, config: dict, name: str, instruction: str
    ) -> None:
        language = config.get("language", "vi")
    ...
    def list_personas(self, console: Console, config: dict) -> None:
    ...
    def remove_persona(
        self, console: Console, config: dict, name: str
    ) -> None:
        personas = config.get("personas", {})
        language = config.get("language", "vi")
        if name not in personas:
            console.print(
                i18n.tr(language, "config_persona_not_found", name=name)
            )
            return
    ...
    def save_profile(self, console: Console, config: dict, name: str) -> None:
        language = config.get("language", "vi")
        ...
        self._repository.save(config)
        console.print(i18n.tr(language, "config_profile_saved", name=name))

    def list_profiles(self, console: Console, config: dict) -> None:
        language = config.get("language", "vi")
        profiles = config.get("profiles") or {}
    ...
    def remove_profile(self, console: Console, config: dict, name: str) -> None:
        language = config.get("language", "vi")
        profiles = config.get("profiles") or {}
        ...
        config["profiles"] = profiles
        self._repository.save(config)
        console.print(i18n.tr(language, "config_profile_removed", name=name))

    def apply_profile(self, console: Console, config: dict, name: str) -> None:
        language = config.get("language", "vi")
        profiles = config.get("profiles") or {}
    ...
    def show_diagnostics(self, console: Console, config: dict) -> None:
        ...
        try:
            ...
            console.print(
                i18n.tr(
                    language,
                    "diagnostics_google_keys",
                    count=len(google_keys),
                )
            )
            console.print(
                i18n.tr(
                    language,
                    "diagnostics_deepseek_keys",
                    count=len(deepseek_keys),
                )
            )
            console.print(
                i18n.tr(
                    language,
                    "diagnostics_groq_keys",
                    count=len(groq_keys),
                )
            )
            console.print(
        ...
        if isinstance(agent_model, str) and (
            agent_model.startswith("deepseek-")
            or agent_model.startswith("groq-")
        ):
        ...
        if language == "vi":
            ...
            console.print(
                f"[dim]Mini-agent HTTP (single-turn): {status}, {len(rules)} rule, "
                f"tools: {', '.join(tools) or '-'}[/dim]"
            )
        else:
            status = "enabled" if enabled else "disabled"
            console.print(
                f"[dim]HTTP mini-agent (single-turn): {status}, {len(rules)} rules, "
                f"tools: {', '.join(tools) or '-'}[/dim]"
            )

        issues = mini_agent.validate_mini_agent_rules(config)
        if issues:
            warning_prefix = i18n.tr(
                language, "diagnostics_mini_agent_warnings"
            )
        ...
        if language == "vi":
            console.print(
                "[dim]Mẹo: dùng cờ --mini-agent-off để tạm thởi tắt mini-agent HTTP cho "
                "phiên này (không sửa config.json).[/dim]"
            )
        else:
            console.print(
                "[dim]Hint: use --mini-agent-off to temporarily disable the HTTP "
                "mini-agent for this run (without changing config.json).[/dim]"
            )
